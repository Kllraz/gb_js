...
function showCategory(category) {
    let productMarkup = getProductMarkup(products[category]);
    products_div.insertAdjacentHTML('afterbegin', productMarkup);
}
...
function getProductMarkup(product) {
    ...
    product.forEach(function (product_item) {
        markup += `<div class="product">
            <div>${product_item.name}</div>
            <img src="${product_item.imageUrl}" alt="">
            <div>${product_item.price}</div>
            <a href="https://example.com/producs/${product_item.id}">Подробнее</a>
        </div>`
    });
